 ...
 * <p>
 * 引入租户类型，默认当前租户
 * - {@link TenantType#ALL} 所有租户，sql解析后设置租户ID为操作用户可操作的租户ID
 * - {@link TenantType#CURRENT} 当前租户，sql解析后设置租户ID为操作用户当前的租户ID
 * - {@link TenantType#OPERATE} 被操作租户，sql解析后设置租户ID为操作的租户ID
 * 应该在设置时校验是否有权限操作该租户
 * <p>
 * 引入rpc传播方式
 * - true，rpc调用传播当前解析模式
 ...
 * - {@link DataScopeType#COMPOSITE_LEVEL} 数据范围，sql解析后设置数据范围为操作用户当前的数据范围
 * - {@link DataScopeType#SELF} 自身，sql解析后设置数据范围为操作用户当前的数据范围
 * <p>
...
@Documented
public @interface TenantParserMode {
